...
Quartenion::Quartenion() :q(0) {
	Q = new Vector<double>({ 0, 0, 0 });
}

Quartenion::Quartenion(double l0, double l1, double l2, double l3): q(l0){
	Q = new Vector<double>({ l1,l2,l3 });
}
...
Quartenion operator*(double s, const Quartenion& quar){
	return quar*s;
};
...
double Quartenion::scal() const{
	return q;
};

Vector<double> Quartenion::vec() const {
	return *Q;
}
...
Quartenion Quartenion::fromKrylovAngles(double yaw, double pitch, double roll){
	double q0 = cos(yaw / 2) * cos(pitch / 2) * cos(roll / 2) - sin(yaw / 2) * sin(pitch / 2) * sin(roll / 2);
	double q1 = sin(yaw / 2) * sin(pitch / 2) * cos(roll / 2) + cos(yaw / 2) * cos(pitch / 2) * sin(roll / 2);
	double q2 = sin(yaw / 2) * cos(pitch / 2) * cos(roll / 2) + cos(yaw / 2) * sin(pitch / 2) * sin(roll / 2);
	double q3 = cos(yaw / 2) * sin(pitch / 2) * cos(roll / 2) - sin(yaw / 2) * cos(pitch / 2) * sin(roll / 2);

	return Quartenion(q0, q1, q2, q3);
};
